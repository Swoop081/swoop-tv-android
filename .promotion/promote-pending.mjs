...
profiles=replaceOnce(profiles,"export function makeProfile({id='',name='Profile',avatar='lion',kids=false,pinHash='',pinSalt='',myList=[],continueWatching=[],watchHistory=[],recentLive=[],liveFavourites=[],providerMode='shared',privateProviders=[],profileSettings={}}={}){","export function makeProfile({id='',name='Profile',avatar='lion',kids=false,pinHash='',pinSalt='',myList=[],continueWatching=[],watchHistory=[],recentLive=[],liveFavourites=[],providerMode='private',privateProviders=[],profileSettings={}}={}){",'new profiles default private');
...
tests=replaceOnce(tests,"if (!appSource.includes('state.sharedProviders=') || !appSource.includes(\"profile.providerMode==='private'\")) throw new Error('Household/private provider scope migration missing');","if (!appSource.includes('state.sharedProviders=') || !appSource.includes(\"profile.providerMode==='private'\")) throw new Error('Household/private provider scope migration missing');\nif (!profilesSource.includes(\"providerMode='private'\")) throw new Error('Secondary account profile constructor is not privacy-first');\nif (!appSource.includes(\"const PROVIDER_ACCOUNT_SCHEMA=2\") || !appSource.includes(\"i===0?{...p,providerMode:'shared'}:{...p,providerMode:'private'\")) throw new Error('Secondary-account privacy migration missing');\nif (!appSource.includes(\"return profile.providerMode==='shared'?'shared':'private'\")) throw new Error('Secondary account provider mode does not fail closed to private');\nif (!appSource.includes('Private providers · Recommended') || !appSource.includes('Use shared household providers')) throw new Error('Privacy-first provider choice UI missing');",'privacy regression guards');
