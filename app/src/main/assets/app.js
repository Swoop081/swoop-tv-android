...
const ANDROID_CURRENT_VERSION='0.8.53';
...
const ANDROID_CURRENT_CHANGELOG=[
  ...
  'Adds generous bottom safe space to browsing pages so the final row never ends hard against the bottom of the TV.',
  'Simplifies What’s New to one close action and removes the redundant My List badge from posters.'
];
let installSeedCache=null,performancePackInfo=null,performancePackPreparing=false;
async function refreshPerformancePackInfo(){try{performancePackInfo=await performancePackStatus()}catch{}return performancePackInfo}
const installSeedPromise=loadInstallSeedCache().then(seed=>{installSeedCache=seed;if(NATIVE_ANDROID&&seed)setTimeout(async()=>{try{await bootstrapPerformancePack(seed);await refreshPerformancePackInfo()}catch{}},0);return seed}).catch(()=>null);
...
const ARTWORK_PREWARM_MEMORY_LIMIT=NATIVE_ANDROID?48:260;
...
function cacheCurrentPageView(){
  ...
  if(state.page==='myswoop'&&mySwoopHeroTimer){clearInterval(mySwoopHeroTimer);mySwoopHeroTimer=null}
...
function resumePageWork(page,root=document){
  ...
  else if(page==='myswoop')scheduleMySwoopHeroRotation();
...
function rememberArtworkPrewarm(url){
  ...
  try{const img=new Image();img.decoding='async';img.onload=img.onerror=()=>{const hit=artworkPrewarmPool.get(url);if(hit)hit.loadedAt=Date.now()};img.src=url;artworkPrewarmPool.set(url,{img,loadedAt:0});trimArtworkPrewarmPool()}catch{}
}
...
async function preloadCriticalArtwork(items=[],limit=36,timeoutMs=4200){
  ...
  let timer=null;const all=Promise.all(urls.map(url=>new Promise(resolve=>{try{if(artworkPrewarmPool.has(url)){resolve();return}const img=new Image();let done=false;const finish=()=>{if(done)return;done=true;artworkPrewarmPool.set(url,{img,loadedAt:Date.now()});trimArtworkPrewarmPool();resolve()};img.onload=()=>{if(typeof img.decode==='function')img.decode().then(finish).catch(finish);else finish()};img.onerror=finish;img.decoding='async';img.src=url;if(img.complete)finish();setTimeout(finish,timeoutMs)}catch{resolve()}})));
...
async function prepareAndroidPerformancePack(onProgress=null,{initial=false}={}){
  if(!NATIVE_ANDROID||performancePackPreparing)return performancePackInfo;performancePackPreparing=true;
  try{
    const seed=await getInstallSeedCache();if(seed)await bootstrapPerformancePack(seed).catch(()=>null);
    const groups=new Map();for(const item of state.catalog||[]){const id=String(item?.providerId||state.provider?.id||'');if(!id)continue;if(!groups.has(id))groups.set(id,[]);groups.get(id).push(item)}
    const entries=[...groups.entries()];for(let i=0;i<entries.length;i++){
      const [providerId,items]=entries[i],provider=providerById(providerId),label=provider?.name||`Provider ${i+1}`;
      await syncProviderPerformancePack(providerId,items,{initial,onProgress:info=>{const local=info.phase==='compare'?Number(info.progress||0)*.2:.2+Number(info.progress||0)*.8;try{onProgress?.({providerId,label,providerIndex:i,totalProviders:entries.length,progress:(i+local)/Math.max(1,entries.length),detail:info.phase==='artwork'?`Caching artwork for ${label}…`:`Comparing ${label} with your saved library…`})}catch{}}}).catch(()=>null);
      await new Promise(r=>setTimeout(r,0));
    }
    return await refreshPerformancePackInfo();
  }finally{performancePackPreparing=false}
}
function performancePackSummary(){const info=performancePackInfo;if(!info)return'Performance Pack prepares local artwork and reusable metadata in the background.';return `${Number(info.warmedArtwork||0).toLocaleString()} artwork files cached · ${Number(info.starmeterRows||0).toLocaleString()} STARmeter people retained · ${Number(info.providerCount||0)} provider snapshot${Number(info.providerCount||0)===1?'':'s'}.`}
...
function toggleLiveFavourite(item){if(!item?.id||item.kind!=='live')return;const on=isLiveFavourite(item);state.liveFavourites=on?state.liveFavourites.filter(id=>id!==item.id):[item.id,...state.liveFavourites.filter(id=>id!==item.id)].slice(0,120);persist();toast(on?'Removed from Favorite Channels':'Added to Favorite Channels');render()}
...
const HOME_ROW_DEFS=[
  ...
  {id:'mylist',label:'Favorites',group:'Your Swoop TV',poster:true,page:'mylist'},
...
function watchHistoryItems(){const out=[],seen=new Set();for(const x of [...state.watchHistory].sort((a,b)=>(b.lastPlayed||0)-(a.lastPlayed||0))){const raw=savedItem(x.id)||x.item,item=continueDisplayItem(raw),key=String(item?.parentSeriesId||item?.id||'');if(item&&key&&!seen.has(key)){seen.add(key);out.push(item)}}return out}
...
function card(item,poster=false,opts={}){
  ...
  const liveFav=item.kind==='live'&&isLiveFavourite(item)?'<span class="card-live-fav">★ FAVORITE</span>':'';
  ...
  return `<button class="card ${poster?'poster':'landscape'} ${posterOwnsTitle?'poster-art-title':''} ${item.kind==='live'?'live-card':''} ${rank?'ranked-card':''}" ${action}="${esc(item.id)}"${imdbHydrationAttr} style="--card-bg:${fallback}" aria-label="${esc(displayTitle)}">
    <div class="card-bg"></div>${art}<div class="card-shade"></div>${rankBadge}${liveBadge}${liveFav}${qualityBadge}${sources}${watched}${imdbBadge}
...
function profilesModal(){
  return `<div class="modal-backdrop profile-manage-backdrop" data-close-modal><div class="modal profile-manage-modal" data-modal-card><div class="modal-head"><div><div class="eyebrow">HOUSEHOLD</div><h2>Profiles</h2><p>Each account can use the household providers from the first account, or keep completely separate private Xtream/M3U provider logins.</p></div><button class="icon-btn" data-close>✕</button></div><div class="modal-body"><div class="profile-manage-list">${state.profiles.map(p=>{const t=profileTheme(p);return `<div class="profile-manage-row">${profileAvatarHtml(p,'profile-avatar-lg')}<div><strong>${esc(p.name)}</strong><span>${p.kids?'Kids restrictions on':'Standard profile'}${p.pinHash?' · PIN protected':''} · ${profileProviderMode(p)==='private'?'Private providers':'Shared providers'} · ${esc(t.name)} theme</span></div><button class="btn secondary compact-btn" data-profile-select="${esc(p.id)}">Switch</button><button class="btn secondary compact-btn" data-profile-edit="${esc(p.id)}">Edit</button></div>`}).join('')}</div><button class="btn accent profile-add-btn" data-profile-add>＋ Add Profile</button></div></div></div>`;
}
...
function hero(feature,providerName,rotation={}){
  ...
  const context=rotation.context==='myswoop'?'myswoop':'home',isMySwoop=context==='myswoop';
  ...
  const typeLabel=isMySwoop?(isSeries?'FAVORITE TV SHOW':'FAVORITE MOVIE'):(feature._heroFeed?`${feature._heroFeed}${feature._heroRank?` · #${feature._heroRank}`:''}`:isLive?'LIVE TV':feature.kind==='movie'?'FEATURED MOVIE':'FEATURED SERIES');
  ...
  return `<section class="hero hero-rotating ${isMySwoop?'myswoop-cinematic-hero':''}" ${isMySwoop?'data-myswoop-hero':'data-home-hero'} data-hero-item="${esc(feature.id)}"><div class="hero-media">${art}${poster}<div class="hero-fallback" style="--hero-fallback:${feature.demoColor||'linear-gradient(135deg,#1d2a44,#080a0e)'}"></div></div><div class="hero-vignette"></div>
...
function mySwoopHeroCandidates(){return listItems().filter(item=>item&&['movie','series'].includes(item.kind)).slice(0,12)}
async function replaceMySwoopHero(){
  if(state.page!=='myswoop'||modal||detailItem||playerItem)return;const current=document.querySelector('[data-myswoop-hero]'),pool=mySwoopHeroCandidates();if(!current||!pool.length)return;
  mySwoopHeroIndex=((mySwoopHeroIndex%pool.length)+pool.length)%pool.length;const index=mySwoopHeroIndex,item=pool[index],token=++mySwoopHeroSwapToken;await preloadCriticalArtwork([item],3,NATIVE_ANDROID?850:1400);if(token!==mySwoopHeroSwapToken||state.page!=='myswoop'||modal||detailItem||playerItem)return;
  const liveCurrent=document.querySelector('[data-myswoop-hero]');if(!liveCurrent)return;const wrap=document.createElement('div');wrap.innerHTML=hero(item,providerSummaryName(),{total:pool.length,index,context:'myswoop'});const next=wrap.firstElementChild;if(!next)return;liveCurrent.replaceWith(next);hydrateArtwork(next);bindDynamicCards(next);bindHeroControls(next);
}
function scheduleMySwoopHeroRotation(){
  if(mySwoopHeroTimer){clearInterval(mySwoopHeroTimer);mySwoopHeroTimer=null}
  if(state.page!=='myswoop'||mySwoopHeroCandidates().length<2)return;
  mySwoopHeroTimer=setInterval(()=>{if(document.hidden||state.page!=='myswoop'||modal||detailItem||playerItem)return;const pool=mySwoopHeroCandidates();if(pool.length<2)return;mySwoopHeroIndex=(mySwoopHeroIndex+1)%pool.length;replaceMySwoopHero()},HERO_ROTATION_MS);
}
...
function continueDisplayItem(item){
  ...
  const seasonPoster=item.seriesPoster||parent?.logo||item.seasonPoster||'';
...
function prewarmPreparedStarmeterArtwork(){
  ...
  const hot=starmeterPeople.slice(0,12);for(const entry of hot){const person=starmeterPersonCache.get(starmeterNormalize(entry.name))?.person||starmeterPersonSeed(entry),portrait=artworkWarmEntry(person?.profile,'w185');if(portrait)rememberArtworkPrewarm(portrait);const cached=starmeterPersonCache.get(starmeterNormalize(entry.name));for(const item of starmeterPersonTitles(cached).slice(0,2)){const poster=artworkWarmEntry(visualItem(item)?.logo,'w185');if(poster)rememberArtworkPrewarm(poster)}}
}
...
function applyPreparedStarmeterResults(people=[],results=[]){
  ...
  for(const row of results){const source=byKey.get(row?.key);if(!source)continue;const value={person:source.person,movies:Array.isArray(row.movies)?sortPersonLibraryItems(row.movies):[],shows:Array.isArray(row.shows)?sortPersonLibraryItems(row.shows):[],loadedAt:Date.now(),prelogin:true};starmeterPersonCache.set(row.key,value);starmeterHotCache.set(row.key,{...(starmeterHotCache.get(row.key)||{}),...value});personLibraryCache.set(`${value.person.id||''}|${String(value.person.name||'').toLowerCase()}`,value);if(state.page==='starmeter')scheduleStarmeterPersonPatch(source.rank);applied++}
  return applied;
}
function applyPersistedStarmeterRows(people=[],rows=[]){
  const byKey=new Map(people.map(row=>[row.key,row]));let applied=0;
  for(const row of rows||[]){const source=byKey.get(row?.key);if(!source)continue;const person={...source.person,...(row.person||{}),name:source.person.name||row.person?.name||''},value={person,movies:Array.isArray(row.movies)?sortPersonLibraryItems(row.movies):[],shows:Array.isArray(row.shows)?sortPersonLibraryItems(row.shows):[],loadedAt:Number(row.loadedAt||Date.now()),persisted:true,providerFingerprint:String(row.providerFingerprint||'')};starmeterPersonCache.set(row.key,value);starmeterHotCache.set(row.key,{...(starmeterHotCache.get(row.key)||{}),...value});personLibraryCache.set(`${person.id||''}|${String(person.name||'').toLowerCase()}`,value);applied++}
  return applied;
}
async function revalidatePersistedStarmeterRows(rows=[],providerFingerprintValue=''){
  if(!NATIVE_ANDROID||!rows.length||!tvCatalogWorkerReady)return;let offset=0,idleWaits=0;
  while(offset<rows.length){
    const busy=starmeterHeldDirectional||starmeterHeldKeys.size>0||(state.page==='starmeter'&&performance.now()-Number(tvLastActivationAt||0)<600);
    if(busy){if(++idleWaits>30)return;await new Promise(r=>setTimeout(r,700));continue}
    idleWaits=0;const chunk=rows.slice(offset,offset+STARMETER_PRELOGIN_BATCH_SIZE),batch=await tvCatalogWorkerRequest('person-match-batch',{people:chunk},12000),results=Array.isArray(batch?.results)?batch.results:[];
    if(results.length){applyPreparedStarmeterResults(chunk,results);const persisted=results.map(row=>{const source=chunk.find(x=>x.key===row.key);return source?{key:row.key,person:source.person,movies:Array.isArray(row.movies)?row.movies:[],shows:Array.isArray(row.shows)?row.shows:[],loadedAt:Date.now()}:null}).filter(Boolean);await savePerformancePackStarmeter(persisted,providerFingerprintValue).catch(()=>false)}
    offset+=chunk.length;await new Promise(r=>setTimeout(r,350));
  }
}
async function prepareStarmeterBeforeLogin(){
  ...
  starmeterBackgroundPreparePromise=(async()=>{
    ...
    try{
      ...
      const providerFingerprintValue=await performancePackProviderFingerprint(enabledProviders().map(p=>p.id)).catch(()=>''),persisted=await loadPerformancePackStarmeter(people.map(x=>x.key)).catch(()=>[]);matched=applyPersistedStarmeterRows(people,persisted);
      if(matched)setStarmeterBackgroundProgress(12+(matched/Math.max(1,total))*50,`Restored ${matched} of ${total} STARmeter people from your Performance Pack…`);
      setStarmeterBackgroundProgress(Math.max(starmeterBackgroundProgress,18),'Restoring your provider catalogue…');
      ...
      const missing=people.filter(row=>!starmeterPersonCache.has(row.key)),persistedByKey=new Map(persisted.map(row=>[row.key,row])),stale=people.filter(row=>!missing.includes(row)&&providerFingerprintValue&&persistedByKey.get(row.key)?.providerFingerprint!==providerFingerprintValue);
      starmeterBackgroundReady=true;starmeterBackgroundComplete=false;
      if(!missing.length){starmeterBackgroundComplete=true;starmeterPreparedProviderSignature=currentSignature;setStarmeterBackgroundProgress(100,'STARmeter Top 100 ready from Performance Pack');prewarmPreparedStarmeterArtwork();if(stale.length)setTimeout(()=>revalidatePersistedStarmeterRows(stale,providerFingerprintValue).catch(()=>null),1800);return true}
      setStarmeterBackgroundProgress(Math.max(28,starmeterBackgroundProgress),`Matching ${missing.length} new STARmeter ${missing.length===1?'person':'people'}…`);
      for(let offset=0;offset<missing.length;offset+=STARMETER_PRELOGIN_BATCH_SIZE){
        const chunk=missing.slice(offset,offset+STARMETER_PRELOGIN_BATCH_SIZE),batch=await tvCatalogWorkerRequest('person-match-batch',{people:chunk},12000),results=Array.isArray(batch?.results)?batch.results:[];
        const applied=applyPreparedStarmeterResults(chunk,results);matched+=applied;
        if(results.length){const persistRows=results.map(row=>{const source=chunk.find(x=>x.key===row.key);return source?{key:row.key,person:source.person,movies:Array.isArray(row.movies)?row.movies:[],shows:Array.isArray(row.shows)?row.shows:[],loadedAt:Date.now()}:null}).filter(Boolean);await savePerformancePackStarmeter(persistRows,providerFingerprintValue).catch(()=>false);await refreshPerformancePackInfo()}
        setStarmeterBackgroundProgress(28+(matched/Math.max(1,total))*62,`STARmeter readying ${matched} of ${total} people…`);
        if(applied!==chunk.length)throw new Error(`STARmeter background match paused at ${matched} of ${total}`);
        await new Promise(resolve=>setTimeout(resolve,0));
      }
      starmeterBackgroundComplete=true;starmeterPreparedProviderSignature=currentSignature;setStarmeterBackgroundProgress(100,'STARmeter Top 100 ready');prewarmPreparedStarmeterArtwork();if(stale.length)setTimeout(()=>revalidatePersistedStarmeterRows(stale,providerFingerprintValue).catch(()=>null),2200);
      return true;
    }catch(err){
      ...
      setStarmeterBackgroundProgress(Math.max(starmeterBackgroundProgress,28+(count/denom)*62),count?`STARmeter is usable now · ${count} of ${denom} retained; finishing in background.`:'STARmeter is usable now · provider matching will retry in the background.');
      scheduleStarmeterBackgroundRetry(5000);return false;
...
function starmeterSectionIsActive(section){return Boolean(section&&document.activeElement&&section.contains(document.activeElement))}
...
function scheduleStarmeterPatchFlush(delay=180){
...
function mySwoopPage(){
  const continuing=continueItems(),saved=listItems(),recent=watchHistoryItems(),liveSaved=state.liveFavourites.map(id=>savedItem(id)).filter(Boolean),heroPool=mySwoopHeroCandidates();
  if(heroPool.length)mySwoopHeroIndex=((mySwoopHeroIndex%heroPool.length)+heroPool.length)%heroPool.length;const heroItem=heroPool[mySwoopHeroIndex]||null;
  const sections=[
    ...
    saved.length?rail('Favorites',saved.slice(0,100),true,`${saved.length.toLocaleString()} favorites`,{rowId:'myswoop-saved',total:saved.length}):'',
    liveSaved.length?liveSimpleRail('Favorite Channels',liveSaved.slice(0,100),`${liveSaved.length.toLocaleString()} saved`):'',
  ...
  const total=continuing.length+saved.length+liveSaved.length+recent.length,header=heroItem?hero(heroItem,providerSummaryName(),{total:heroPool.length,index:mySwoopHeroIndex,context:'myswoop'}):`<section class="collection-hero myswoop-hero"><div class="eyebrow">YOUR SWOOP TV</div><h1>My SwoopTV</h1><p>Your Continue Watching, Favorites, favorite channels and recent watches all live here.</p><div class="collection-count">${total.toLocaleString()} personal items</div></section>`;
  return `<main class="page myswoop-page">${header}<div class="page-content myswoop-content">${sections||empty('My SwoopTV is ready','Start watching, add a title to Favorites or favorite a Live TV channel and it will appear here.')}</div></main>`;
}
...
function startAndroidBootFunTicker(){
  ...
  tick();androidBootFunTimer=setInterval(()=>{androidBootFunIndex=(androidBootFunIndex+1)%ANDROID_BOOT_FUN_LINES.length;tick()},15000);
}
...
function whatsNewModal(){
  ...
  return `<div class="modal-backdrop whats-new-backdrop"><div class="modal whats-new-modal" data-modal-card role="dialog" aria-modal="true" aria-labelledby="whatsNewTitle"><div class="modal-head"><div><div class="eyebrow">WHAT'S NEW</div><h2 id="whatsNewTitle">Swoop TV v${esc(ANDROID_CURRENT_VERSION)}</h2><p>Hardware stability, Favorites and TV presentation improvements.</p></div></div><div class="modal-body whats-new-body">${entries.length?`<ul>${entries.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:`<ul><li>Faster TV navigation and lower renderer memory use.</li><li>Favorites-first My SwoopTV and a persistent Live TV preview header.</li></ul>`}<div class="cta-row"><button class="btn accent" data-whats-new-done autofocus>Got it</button></div></div></div></div>`;
}
...
function toggleMyList(item){if(!item)return;clearPersistentPageViews(['home','myswoop','mylist','movies','series']);const ids=new Set(logicalItemIds(item)),saved=state.myList.some(id=>ids.has(id));if(saved){state.myList=state.myList.filter(id=>!ids.has(id));toast('Removed from Favorites')}else{state.myList.unshift(item.id);toast('Added to Favorites')}persist();if(detailItem)patchDetailSectionsFromState({controls:true});else render()}
...
async function refreshProvider(id,{quiet=false,manageTask=true,onProgress=null,deferPersist=false}={}){
  ...
  try{
    ...
    replaceProviderCatalog(p.id,resultItems);report(81,'Updating your library…');if(NATIVE_ANDROID){const delta=await syncProviderPerformancePack(p.id,resultItems,{onProgress:info=>{const pct=82+Math.round(Math.max(0,Math.min(1,Number(info.progress||0)))*14);report(Math.min(96,pct),info.phase==='artwork'?`Caching artwork for changed titles · ${Number(info.processed||0).toLocaleString()} / ${Number(info.total||0).toLocaleString()}`:'Comparing with your saved provider snapshot…')}}).catch(()=>null);if(delta)report(97,`${Number(delta.added||0).toLocaleString()} added · ${Number(delta.changed||0).toLocaleString()} changed · ${Number(delta.removed||0).toLocaleString()} removed`);await refreshPerformancePackInfo()}if(NATIVE_WINDOWS){await nativeCatalogReplaceProvider(p.id,resultItems,{onProgress:info=>{const pct=82+Math.round((Number(info.loaded||0)/Math.max(1,Number(info.total||1)))*14);report(Math.min(96,pct),`Saving ${Number(info.loaded||0).toLocaleString()} of ${Number(info.total||0).toLocaleString()} items…`)}});report(97,'Finishing your library…');await activateNativeCatalogIfAvailable();}Object.assign(p,{lastRefreshed:Date.now(),lastError:'',counts,...(p.type==='xtream'?{expiresAt,expiryNever}:{})});
...
function bind(){
  ...
  const optimisePack=document.querySelector('[data-performance-pack-optimize]');if(optimisePack)optimisePack.onclick=async()=>{taskProgressStart({title:'Optimising Swoop TV…',detail:'Comparing your saved library and caching missing artwork.',progress:0});try{await prepareAndroidPerformancePack(info=>taskProgressUpdate({title:`Optimising ${info.label||'your library'}…`,detail:info.detail||'Preparing the local Performance Pack…',progress:Math.round(Math.max(0,Math.min(1,Number(info.progress||0)))*100)}));await prepareStarmeterBeforeLogin().catch(()=>false);await refreshPerformancePackInfo();taskProgressEnd({success:true,title:'Performance Pack ready',detail:performancePackSummary(),hold:1800});render()}catch(err){taskProgressEnd({success:false,title:'Optimisation paused',detail:err?.message||'Swoop TV will retry this work in the background.',hold:2200})}};
  ...
  document.querySelector('#m3uForm')?.addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),file=fd.get('file'),url=String(fd.get('url')||'').trim(),name=String(fd.get('name')||'M3U Provider').trim()||'M3U Provider',epgUrl=String(fd.get('epgUrl')||'').trim(),remember=Boolean(fd.get('remember'));const firstRun=!state.profiles.length;providerProgressStart('m3u',name);if(firstRun){firstRunProviderBusy=true;firstRunProviderReady=false;firstRunProviderError='';firstRunStage='avatar';modal=null;profilePickerOpen=true;render();}try{providerProgressUpdate({step:'read',progress:12,title:`Reading ${name}…`,detail:file&&file.size?'Loading your playlist.':'Loading your playlist.'});let text;if(file&&file.size)text=await file.text();else if(url){if(NATIVE_PLAYBACK)text=await nativeFetchText(url);else{const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error(`Playlist returned HTTP ${r.status}`);text=await r.text()}}else throw new Error('Choose an M3U file or enter a playlist URL.');providerProgressMark('read','Complete');providerProgressUpdate({step:'parse',progress:55,title:'Parsing channels…',detail:'Preparing channels.'});await new Promise(r=>setTimeout(r,40));const providerId=scopedProviderId('m3u',`${url||name}`),cat=parseM3U(text,providerId).filter(x=>!isDemoItem(x));if(!cat.length)throw new Error('No playable entries were found in that M3U playlist.');providerProgressMark('parse',`${cat.length.toLocaleString()} items`);providerProgressUpdate({step:'save',progress:86,title:'Adding provider…',detail:`Saving your library.`});const counts={live:cat.filter(x=>x.kind==='live').length,movie:cat.filter(x=>x.kind==='movie').length,series:cat.filter(x=>x.kind==='series').length};replaceProviderCatalog(providerId,cat);if(NATIVE_ANDROID){providerProgressUpdate({step:'save',progress:90,title:'Optimising Swoop TV…',detail:'Building the reusable local library and caching priority artwork.'});await syncProviderPerformancePack(providerId,cat,{initial:true,onProgress:info=>{const pct=90+Math.round(Math.max(0,Math.min(1,Number(info.progress||0)))*7);providerProgressUpdate({step:'save',progress:pct,title:'Optimising Swoop TV…',detail:info.phase==='artwork'?`Caching artwork · ${Number(info.processed||0).toLocaleString()} / ${Number(info.total||0).toLocaleString()}`:'Comparing this provider with your saved library…'})}}).catch(()=>null);await refreshPerformancePackInfo()}if(NATIVE_WINDOWS){providerProgressUpdate({step:'save',progress:91,title:'Saving your library…',detail:'Saving your provider library.'});await nativeCatalogReplaceProvider(providerId,cat,{onProgress:info=>providerProgressUpdate({step:'save',progress:91+Math.round((info.loaded/Math.max(1,info.total))*6),stepDetail:`${info.loaded.toLocaleString()} / ${info.total.toLocaleString()} saved`})});await activateNativeCatalogIfAvailable();}upsertProviderRecord({id:providerId,type:'m3u',name,url,epgUrl,enabled:true,status:'connected',lastRefreshed:Date.now(),counts});if(remember&&url)saveProviderCredentials({id:providerId,type:'m3u',name,url,epgUrl,enabled:true,priority:providerById(providerId)?.priority??state.providers.length-1,lastRefreshed:Date.now(),counts});else if(url)sessionProviderConfigs.set(providerId,{id:providerId,type:'m3u',name,url,epgUrl});state.mdblistRows.forEach(r=>{r.items=[];r.updatedAt=0;r.error=''});state.webDiscovery={};m3uGuideTextCache.delete(providerId);xtreamGuideTextCache.delete(providerId);guideProviderCategoryCache.key='';guideProviderCategoryCache.names=[];guideChannelCache.key='';guideChannelCache.items=[];await persist(NATIVE_WINDOWS?'cache':true);providerProgressMark('save','Ready');if(firstRun){firstRunProviderBusy=false;firstRunProviderReady=true;firstRunProviderError='';firstRunStage='avatar';void completeFirstRunIfReady();if(!firstRunAvatarConfirmed)render()}else providerProgressSuccess(`${name} added · ${counts.live.toLocaleString()} live · ${counts.movie.toLocaleString()} movies · ${counts.series.toLocaleString()} shows`)}catch(err){if(firstRun){firstRunProviderBusy=false;firstRunProviderReady=false;firstRunProviderError=err.message||String(err);firstRunStage='provider';firstRunAvatarConfirmed=false;modal='provider';profilePickerOpen=true;render();requestAnimationFrame(()=>setStatus('#providerStatus',firstRunProviderError,'err'))}else providerProgressError(err.message||String(err))}});
  document.querySelector('#xtreamForm')?.addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),relayUrl=String(fd.get('relayUrl')||'').trim(),relayToken=String(fd.get('relayToken')||''),name=String(fd.get('name')||'Xtream Provider').trim()||'Xtream Provider',cfg={server:String(fd.get('server')).trim(),username:String(fd.get('username')),password:String(fd.get('password')),relayUrl,relayToken};const firstRun=!state.profiles.length;providerProgressStart('xtream',name);try{providerProgressUpdate({step:'contact',progress:7,title:`Contacting ${name}…`,detail:NATIVE_PLAYBACK?'Connecting to this provider.':relayUrl?'Connecting to this provider.':'Connecting directly to this Xtream server.'});const profile=await testXtream(cfg),expiresAt=providerExpiryFromProfile(profile),expiryNever=!expiresAt&&String(profile?.user_info?.exp_date??'').trim()==='0';providerProgressMark('contact','Reached');providerProgressUpdate({step:'auth',progress:18,title:'Verifying your Xtream login…',detail:'Checking that the account is active and authorised.'});if(String(profile?.user_info?.auth)==='0')throw new Error('Xtream account was not authorised.');providerProgressMark('auth','Authorised');if(firstRun){firstRunProviderBusy=true;firstRunProviderReady=false;firstRunProviderError='';firstRunStage='avatar';modal=null;profilePickerOpen=true;render();}providerProgressUpdate({step:'live',progress:26,title:'Loading this provider library…',detail:'Live TV, Movies and TV Shows are loading. Your existing providers remain available.'});const providerId=scopedProviderId('xtream',`${cfg.server}|${cfg.username}`),completedSections=new Set();const result=await importXtream(cfg,providerId,info=>{if(info?.section){completedSections.add(info.section);providerProgressMark(info.section,`${Number(info.count||0).toLocaleString()} items`);const next=['live','movie','series'].find(x=>!completedSections.has(x))||'save',progress=next==='live'?30:next==='movie'?47:next==='series'?64:80,nextLabel=next==='live'?'Live TV':next==='movie'?'Movies':next==='series'?'TV Shows':'your library';providerProgressUpdate({step:next,progress,title:next==='save'?'Provider loaded — finishing setup…':`Loading ${nextLabel}…`,detail:next==='save'?'Finishing setup.':'The remaining sections are still loading.'})}});if(!result.items.length)throw new Error('Connected, but this provider returned no playable content.');providerProgressUpdate({step:'save',progress:88,title:'Adding provider…',detail:'Saving your library.'});const remember=Boolean(fd.get('remember')),counts=result.counts||{live:result.items.filter(x=>x.kind==='live').length,movie:result.items.filter(x=>x.kind==='movie').length,series:result.items.filter(x=>x.kind==='series').length};replaceProviderCatalog(providerId,result.items);if(NATIVE_ANDROID){providerProgressUpdate({step:'save',progress:90,title:'Optimising Swoop TV…',detail:'Comparing your library and caching artwork for fast browsing.'});await syncProviderPerformancePack(providerId,result.items,{initial:true,onProgress:info=>{const pct=90+Math.round(Math.max(0,Math.min(1,Number(info.progress||0)))*7);providerProgressUpdate({step:'save',progress:pct,title:'Optimising Swoop TV…',detail:info.phase==='artwork'?`Caching artwork · ${Number(info.processed||0).toLocaleString()} / ${Number(info.total||0).toLocaleString()}`:'Comparing this provider with your saved library…'})}}).catch(()=>null);await refreshPerformancePackInfo()}if(NATIVE_WINDOWS){providerProgressUpdate({step:'save',progress:90,title:'Saving your library…',detail:'Saving your provider library.'});await nativeCatalogReplaceProvider(providerId,result.items,{onProgress:info=>providerProgressUpdate({step:'save',progress:90+Math.round((info.loaded/Math.max(1,info.total))*7),stepDetail:`${info.loaded.toLocaleString()} / ${info.total.toLocaleString()} saved`})});await activateNativeCatalogIfAvailable();}upsertProviderRecord({id:providerId,type:'xtream',name,server:cfg.server,connection:NATIVE_WINDOWS?'windows-native':NATIVE_ANDROID?'android-native':relayUrl?'helper':'direct',relayUrl,enabled:true,status:'connected',lastRefreshed:Date.now(),counts,expiresAt,expiryNever});sessionProviderConfigs.set(providerId,{...cfg,id:providerId,type:'xtream',name,expiresAt,expiryNever});if(remember)saveProviderCredentials({id:providerId,type:'xtream',name,...cfg,enabled:true,priority:providerById(providerId)?.priority??state.providers.length-1,lastRefreshed:Date.now(),counts,expiresAt,expiryNever});if(profileProviderMode()==='shared'){state.settings.xtreamRelayUrl=relayUrl||state.settings.xtreamRelayUrl;state.settings.xtreamRelayToken=remember&&relayToken?relayToken:state.settings.xtreamRelayToken;}state.mdblistRows.forEach(r=>{r.items=[];r.updatedAt=0;r.error=''});state.webDiscovery={};await persist(NATIVE_WINDOWS?'cache':true);providerProgressMark('save','Ready');if(firstRun){firstRunProviderBusy=false;firstRunProviderReady=true;firstRunProviderError='';firstRunStage='avatar';void completeFirstRunIfReady();if(!firstRunAvatarConfirmed)render()}else providerProgressSuccess(`${name} added · ${counts.live.toLocaleString()} live · ${counts.movie.toLocaleString()} movies · ${counts.series.toLocaleString()} shows`)}catch(err){if(firstRun){firstRunProviderBusy=false;firstRunProviderReady=false;firstRunProviderError=err.message||String(err);firstRunStage='provider';firstRunAvatarConfirmed=false;modal='provider';profilePickerOpen=true;render();requestAnimationFrame(()=>setStatus('#providerStatus',firstRunProviderError,'err'))}else providerProgressError(err.message||String(err))}});
...
let tvFocusMemory=null,tvHeldHorizontal=false;
const tvHeldHorizontalKeys=new Set();
...
function tvPrefetchArtworkWindow(current,ahead=18,behind=4){
  ...
  androidStartupGatePromise=(async()=>{
    ...
    try{
      ...
      if(state.catalog.length){
        ...
        updateStartupRefreshProgress({progress:8,provider:'Performance Pack',detail:'Checking saved metadata and artwork…'});
        await prepareAndroidPerformancePack(info=>updateStartupRefreshProgress({progress:8+Math.max(0,Math.min(1,Number(info.progress||0)))*60,provider:`Performance Pack · ${info.label||''}`.replace(/ · $/,''),detail:info.detail||'Optimising your saved library…'})).catch(()=>null);
      ...
      if(state.catalog.length){
        ...
        await persist(true);
        updateStartupRefreshProgress({progress:73,provider:'Performance Pack',detail:'Caching priority artwork and retaining metadata for future launches…'});
        await prepareAndroidPerformancePack(info=>updateStartupRefreshProgress({progress:73+Math.max(0,Math.min(1,Number(info.progress||0)))*12,provider:`Performance Pack · ${info.label||''}`.replace(/ · $/,''),detail:info.detail||'Optimising your library…'}),{initial:true}).catch(()=>null);
        updateStartupRefreshProgress({progress:85,provider:'Preparing Home',detail:'Building your Home screen…'});
        await prepareAndroidHomeBeforeEntry(info=>{
          const mapped=85+Math.max(0,Math.min(1,Number(info.progress||0)))*13;
          updateStartupRefreshProgress({progress:mapped,provider:info.label||'Preparing Home',detail:info.detail||'Preparing Home…'});
        });
