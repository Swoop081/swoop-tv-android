...
const CREDIT_PERSON_LIMIT=Math.max(0,Math.min(100,Number(process.env.SWOOP_SEED_CREDIT_PEOPLE||24)));
...
function compactCredit(raw={}){const ids=raw.ids&&typeof raw.ids==='object'?raw.ids:{};const media=String(raw.media_type||raw.mediaType||'');return {media_type:media,id:raw.id??'',title:String(raw.title||''),name:String(raw.name||''),year:String(raw.year||compactYear(raw.release_date||raw.first_air_date||'')),release_date:String(raw.release_date||''),first_air_date:String(raw.first_air_date||''),tmdb_id:raw.tmdb_id??raw.tmdb??ids.tmdb??'',imdb_id:String(raw.imdb_id??raw.imdb??ids.imdb??''),popularity:Number(raw.popularity||0)||0,vote_count:Number(raw.vote_count||0)||0}}
