...
async function fetchTraktPublicApiList(mediaType){
  ...
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),25000);
  try{
    const res=await fetch(url,{headers:{accept:'application/json','trakt-api-version':'2','trakt-api-key':TRAKT_PUBLIC_WEB_CLIENT_KEY,'user-agent':'SwoopTV/0.8.44'},signal:controller.signal,redirect:'follow'});
